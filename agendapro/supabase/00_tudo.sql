...
create table if not exists public.convites_equipe (
  ...
  /* ⚠ QUAL FICHA DA AGENDA É ESTE LOGIN — obrigatório para `profissional`.

     Sem isto o convite entregava um painel VAZIO. O RLS decide o que quem
     atende enxerga por `meu_profissional_id()`, que procura a ficha de
     `profissionais` com o `perfil_id` dela. Vínculo sem ficha ligada = função
     devolve NULL = nenhum atendimento passa pela policy.

     Ou seja: a pessoa aceitava o convite, entrava, e via a agenda em branco
     no primeiro dia de trabalho. O login existia e não servia para nada. */
  profissional_id uuid references public.profissionais(id) on delete cascade,
...
-- 2) CRIAR
-- ---------------------------------------------------------------------------
-- A versão de três argumentos existiu antes de o convite saber ligar a ficha
-- da agenda. Deixá-la no banco criaria duas funções com o mesmo nome, e a
-- chamada sem o quarto argumento cairia na antiga — que entrega o painel
-- vazio de novo, em silêncio.
...
begin
  ...
  if p_papel = 'profissional' then
    ...
    -- Ficha de OUTRO salão seria dar acesso à agenda alheia.
    if pr.id is null or pr.salao_id <> p_salao then
      raise exception 'Esta agenda não é deste salão.' using errcode = 'check_violation';
    end if;
    -- E ficha que já tem dono não se entrega duas vezes: seria roubar a
    -- agenda de quem já entra com ela.
    if pr.perfil_id is not null then
      raise exception 'A agenda de % já tem login.', pr.nome
        using errcode = 'check_violation';
    end if;
  else
    p_profissional := null;   -- recepção e admin não têm coluna na agenda
  end if;
...
begin
  ...
  /* Amarra o login à ficha da agenda. É esta linha que faz o RLS entregar os
     atendimentos dela — sem ela, `meu_profissional_id()` devolve NULL e a
     agenda abre em branco.

     `perfil_id is null` na condição: se alguém já ficou com essa ficha entre
     a criação e o aceite, o convite não a rouba. */
